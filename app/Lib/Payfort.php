...
class Payfort
{
     ...
     */
    public $customerEmail      = 'sales@moreideas.ae';
     ...
     */
    public $projectUrlPath     = '/payment_mi';
    ...
    public function processRequest($paymentMethod, $amt, $currency, $email, $order)
    {
        if ($paymentMethod == 'cc_merchantpage' || $paymentMethod == 'cc_merchantpage2' || $paymentMethod == 'installments_merchantpage') {
            $merchantPageData = $this->getMerchantPageData($paymentMethod, $order);
            $postData = $merchantPageData['params'];
            $gatewayUrl = $merchantPageData['url'];
        }
        else{
            $data = $this->getRedirectionData($paymentMethod,$amt, $currency, $email, $order);
    ...
    public function getRedirectionData($paymentMethod, $amt, $currency, $email, $order) {
        $merchantReference = $order; //$this->generateMerchantReference();
        if ($this->sandboxMode) {
        ...
        // }
        $postData = array(
            'amount'              => $this->convertFortAmount($amt, $currency),
            'currency'            => strtoupper($currency),
            'merchant_identifier' => $this->merchantIdentifier,
            'access_code'         => $this->accessCode,
            'merchant_reference'  => $merchantReference,
            'customer_email'      => $email,//$this->customerEmail,
            //'customer_name'         => trim($order_info['b_firstname'].' '.$order_info['b_lastname']),
            'command'             => $this->command,
            'language'            => $this->language,
            'return_url'          => $this->getUrl('payment_route?r=processResponse'),
        );
        ...
        $this->log($debugMsg);
        $this->saveRequestTransaction($postData['merchant_reference'], $postData['currency'], $postData['customer_email'], $postData['command'], $postData['language'], $postData['return_url']);
        return array('url' => $gatewayUrl, 'params' => $postData);
    }

    public function getMerchantPageData($paymentMethod, $order)
    {
        $merchantReference = $order;//$this->generateMerchantReference();
        $returnUrl = $this->getUrl('payment_route?r=merchantPageReturn');
        if(isset($_GET['3ds']) && $_GET['3ds'] == 'no') {
            $returnUrl = $this->getUrl('payment_route?r=merchantPageReturn&3ds=no');
        }
        $iframeParams              = array(
            'merchant_identifier' => $this->merchantIdentifier,
            'access_code'         => $this->accessCode,
        ...
        $this->log($debugMsg);
        $this->saveRequestTransaction($iframeParams['merchant_reference'], $iframeParams['currency'], $iframeParams['customer_email'], $iframeParams['service_command'], $iframeParams['language'], $iframeParams['return_url']);
    ...
    public function processResponse()
    {
        ...
        if(!empty($fortParams)) {
            $payment_option = $fortParams['payment_option'] ?? null;
            $customer_ip = $fortParams['customer_ip'] ?? null;
            $fort_id = $fortParams['fort_id'] ?? null;
            $merchantReference = $fortParams['merchant_reference'] ?? null;
            $this->saveResponseTransaction(
                $merchantReference,
                $fortParams['currency'],
                $fortParams['customer_email'],
                $fortParams['command'],
                $fortParams['language'],
                $fortParams['response_code'],
                $payment_option,
                $customer_ip,
                $fort_id,
                $fortParams['response_message'],
                $fortParams['status']);
        }
        $this->log($debugMsg);
        ...
        return "<html><body onLoad=\"javascript: window.top.location.href='" . $return_url . "'\"></body></html>";
    }
    ...
    {
        $url = 'https://' . $_SERVER['HTTP_HOST'] . $this->projectUrlPath .'/'. $path;
        return $url;
    }
    ...
    public function log($messages) {
        $insert_logs = new PaymentPayfortLogs([
            'messages' => $messages,
        ]);
        $insert_logs->save();
    }
    ...
    function saveRequestTransaction($order_id, $currency, $customer_email, $command, $language, $return_url)
    {
    ...
    function saveResponseTransaction($order_id, $currency, $customer_email, $command, $language, $response_code, $payment_option, $customer_ip, $fort_id, $response_message, $transaction_status)
    {
